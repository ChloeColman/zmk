...
#if IS_ENABLED(CONFIG_NICE_VIEW_GEM_BONGO_CAT)
#include "bongo_cat.h"
#endif
...
static void set_wpm_status(struct zmk_widget_screen *widget, struct wpm_status_state state) {
    ...
    widget->state.wpm[WPM_HISTORY_SIZE - 1] = state.wpm;
...
#if IS_ENABLED(CONFIG_NICE_VIEW_GEM_BONGO_CAT)
/**
 * Keypress listener for bongo cat animation
 **/

struct keypress_state {
    uint32_t counter;
};

static void set_keypress_status(struct zmk_widget_screen *widget, struct keypress_state state) {
    widget->state.keypress_counter = state.counter;
    draw_top(widget->obj, widget->cbuf, &widget->state);
}

static void keypress_status_update_cb(struct keypress_state state) {
    struct zmk_widget_screen *widget;
...
static struct keypress_state keypress_status_get_state(const zmk_event_t *eh) {
    const struct zmk_keycode_state_changed *ev = as_zmk_keycode_state_changed(eh);
    if (ev && ev->state) {  // Only on key down
        keypress_counter++;
    }
    return (struct keypress_state){.counter = keypress_counter};
}

ZMK_DISPLAY_WIDGET_LISTENER(widget_keypress_status, struct keypress_state, keypress_status_update_cb,
